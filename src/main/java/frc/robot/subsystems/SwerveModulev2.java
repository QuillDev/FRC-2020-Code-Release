...
public class SwerveModulev2 {
   ...
   */
  public SwerveModulev2(int driveMotorChannel, int turningMotorChannel, boolean driveEncoderReversed, boolean turningEncoderReversed,boolean inverseTurn, String name) {
  ...
  private double getAngle() {
    double angle = unitsToDegrees(steerEncoder.getQuadraturePosition());
  ...
  public void setDesiredState(SwerveModuleState state) {
    ...
    double error = Math.IEEEremainder( angle - getAngle(), 360);

    double velocity = state.speedMetersPerSecond / Constants.DriveConstants.kMaxSpeedMetersPerSecond * Constants.DriveConstants.speedLimit;

    //if error is more than 90 degrees invert the things
    if(error > 90) {
      error -= 180;
      velocity *= -1;
    }

    //if error is less than -90 also invert the things
    if(error < -90) {
      error += 180;
      velocity *= -1;
    }

    //calculated setpoint in current encoder units
    double setpoint = (getAngle() + error) / 360 * 4096;

    //set the steer motor to the calculated degree using the PID Algorithm from CTRE
    steer.set(ControlMode.MotionMagic, setpoint);
